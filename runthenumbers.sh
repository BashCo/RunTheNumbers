...
do
    ...
    CURRENT_BLOCK="$(curl -s --user "${USERNAME}":"${PASSWORD}" --data-binary '{"jsonrpc": "1.0", "id":"curltest", "method": "getblockcount", "params": [] }' -H 'content-type: text/plain;' http://${IP_ADDRESS}:${PORT}/ | \
    ...
    if [ "$CURRENT_BLOCK" == "$TARGET_BLOCK" ]; then
        ...
        TXOUTSETINFO="$(curl -s --user "${USERNAME}":"${PASSWORD}" --data-binary '{"jsonrpc": "1.0", "id":"curltest", "method": "gettxoutsetinfo", "params": [] }' -H 'content-type: text/plain;' http://${IP_ADDRESS}:${PORT}/ | \
        python -c "import sys, json; parsed = json.load(sys.stdin)['result']; print(json.dumps(parsed, indent=2))")"
        echo
        echo
        echo "${TXOUTSETINFO}"
        echo
